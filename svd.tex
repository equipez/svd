...
\begin{proof}
  ...
  There is a~$y \in \range(A^\hmt)$ such that~$x=Ay$. By assumption, $\|y\|=\|x\|$. Hence
    \begin{equation*}
      ...
      \frac{1}{\|x\|} x^\hmt x\;=\; \|x\|.
    \end{equation*}
  For any~$z\in\CC^{n}$, let~$z'$ be its orthogonal projection on~$\range(A^\hmt)$. Then~$\|Az\|\!=\!\|Az'\|\!=\!\|z'\|\!\le\!\|z\|$. Thus 
    \begin{equation*}
...
\begin{proof}
  ...
  Corollary~\ref{coro:eyfull}, there exists $W_1$, $W_2\in \CC^{m\times m}$ and~$\Sigma \in \real^{m\times n}$
